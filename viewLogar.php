...
switch($_POST['controller'])
{
		...
		else
		{
			foreach($controllerUSUARIO->arrResposta as $dados)
			{
				if(trim($dados['usuario']) == trim($_POST['formLogin_user']) && trim($dados['senha']) == trim($strSenha))
				{
					...
					$msg .='<img  height="200px"  width="200px;" src="http://177.70.26.45/beaverpousada/beaver-room-vertical-transparente.png"></img>';
	...
	case 'montarLink':
		...
		$templet .= '<a class="brand" title="Ir para a tela inicial" href="http://177.70.26.45/beaverpousada/beaver-room-vertical-transparente.png">
		<img  width=140px height=20 style="position:absolute;margin-top:-30px" src="http://177.70.26.45/beaverpousada/beaver-room-vertical-transparente.png"></img></a>';
		...
		*/
		...
		<br><br><br>";
			...
			else
			{
				...
				$strTable .= '<fieldset class=" " style="width:98%">
					   <table class="table table-striped table-bordered table-condensed" style="position:relative" width="100%">
						  <tbody>
							 <tr>
								<td width="5%" align="center"><input id="idmarca" onclick="marcarCheckbox(\'opcaoCheckOut\')" type="checkbox"></td>
								<td align="center">Selecionar todos</td>
							 </tr>
						  </tbody>
					   </table>';
			...
			if($strTable3 == "")
			{
				$strTable .="<div class='alert' aling='center'>";
				$strTable .="<div align='center'>";
				$strTable .="<strong>Aviso!</strong> okokokok não existem registros!";
				$strTable .="</div></div>";
			}
			else
			{
			...
			$strTable .="</div></p>";*/
		$strTable .="<div id='second' class='tab-pane'>";
...
$strTable .= " <fieldset class='moldura ' style='width: 97%'>";

$strTable .= '
	
	<table class="table "  border="0" style="position:relative" width="100%">
   <tbody>
      <tr>
